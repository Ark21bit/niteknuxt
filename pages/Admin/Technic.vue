<template>
    <button-group class="gap-3 w-fit my-6 flex-wrap">
    ...
    <div>
        <div v-if="str == 'edit'">
            <div class="flex items-center gap-3 mb-8 max-md:flex-col">
                <h2 class="h2">Выберите технику для редактирования</h2>     
                <Input size="sm" placeholder="Введите название техники" class="grow max-md:w-full" v-model="filterTitle">
                    <template #prefix>
            ...
            <div>
                <form @submit.prevent="updateTechnic" class="flex flex-col gap-4 md:gap-6">
                    <input type="text" placeholder="Название техники" class="input_primary" v-model="form_technic.title">
                    <input type="text" placeholder="цена за смену" class="input_primary" v-model.number="form_technic.price_smena">
                    <input type="text" placeholder="цена за час" class="input_primary" v-model.number="form_technic.price_hours">
                    <input type="text" placeholder="Производитель" class="input_primary" v-model.number="form_technic.manufacturer">
                    <input type="text" placeholder="ссылка на изображение" class="input_primary" v-model.number="form_technic.img">
                    <textarea type="email" placeholder="Описание техники" class="input_primary" v-model="form_technic.description"/>
                    <select class="input_primary" v-model="form_technic.category_id">
                        <option v-for="category in categorys" :value="category.id" >{{category.title}}</option>
                    </select>
                    <button type="submit" class="btn_primary w-full max-w-[306px] mt-6 m-auto max-md:mt-4" :disabled="selectTechnic == null">Обновить</button>
                </form>
            </div>           
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-5 py-6">
                <div @click="selectTechnicAdd(technic)" :class="{'outline': selectTechnic == technic.id}" v-for="technic in technicsFilter" class="cursor-pointer px-1.5 pb-3 md:p-3 pt-0 bg-white shadow-[0_4px_30px_0_rgba(0,0,0,0.1)] rounded-lg font-light text-sm">
                    <div><img class="object-contain aspect-[4/3]" src="~/assets/img/image2.png" alt=""></div>
                    <p>{{technic.title}}</p>
                </div>
            </div>             
        </div>
        <div v-else-if="str == 'delete'">
            <div class="flex items-center gap-3 mb-8  max-md:flex-col">
                <h2 class="h2">Выберите технику для удаления</h2>
                <Input size="sm" placeholder="Введите название техники" class="grow max-md:w-full" v-model="filterTitle">
                    <template #prefix>
                      <svg aria-hidden="true" class="w-5 h-5 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    </template>
            ...
            <Button color="red" class="focus:ring-0 mb-6" @click="deleteTechnic" :disabled="selectTechnic == null">            
                <template #prefix>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-5">
                <div @click="selectTechnic = technic.id" :class="{'outline': selectTechnic == technic.id}" v-for="technic in technicsFilter" class="cursor-pointer px-1.5 pb-3 md:p-3 pt-0 bg-white shadow-[0_4px_30px_0_rgba(0,0,0,0.1)] rounded-lg font-light text-sm">
                    <div><img class="object-contain aspect-[4/3]" src="~/assets/img/image2.png" alt=""></div>
                    <p>{{technic.title}}</p>
                </div>                
            </div>                    
        ...
        <div v-else-if="str == 'add'" class="mb-6">                        
            <h2 class="h2 mb-8">Добавление техники</h2>
            <div>
                <form @submit.prevent="addTechnic" class="flex flex-col gap-4 md:gap-6">
                    <input type="text" placeholder="Название техники" class="input_primary" v-model="form_technic.title">
                    <input type="text" placeholder="цена за смену" class="input_primary" v-model.number="form_technic.price_smena">
                    <input type="text" placeholder="цена за час" class="input_primary" v-model.number="form_technic.price_hours">
                    <input type="text" placeholder="Производитель" class="input_primary" v-model.number="form_technic.manufacturer">
...
let form_technic = ref({})

const config = useRuntimeConfig()

let selectTechnic = ref(null)

let filterTitle = ref(null)

const technicsFilter = computed( ()=>{    
    if (filterTitle.value) {
        return technics.value.filter(a=>{
...
const selectTechnicAdd = (technic)=>{
    selectTechnic.value = technic.id
    form_technic.value = technic
}

/* const completionForm = (technic_id)=>{
    form_technic.value.title = technics[technic_id]
    form_technic.value.description = technics[technic_id]
    form_technic.value.category_id = technics[technic_id]
    form_technic.value.price_smena = technics[technic_id]
    form_technic.value.price_hours = technics[technic_id]
}
 */
let technics = ref({})


const deleteTechnic = async()=>{    
    const { data, error } = await useFetch(`${config.public.apiBase}/technic/${selectTechnic.value}`,{
        method: 'delete',
        headers: {
        ...
        onResponseError({ request, response, options }) {
            ...
            alertStore.message = "При удалении техники произошла ошибка"
            alertStore.isAlert = true  
        },   
        onResponse({ request, response, options }) {
            alertStore.type = "info"
            alertStore.message = "Техника успешно удалена"
            alertStore.isAlert = true 
            getTechnic()
        },
    })   
...
const addTechnic = async()=>{    
    const { data, error } = await useFetch(`${config.public.apiBase}/technic`,{
        method: 'post',
        headers: {
            Authorization: `Bearer ${accountStore.api_token}`,
        },
        body:{
            title: form_technic.value.title,
            description:form_technic.value.description,
            category_id:form_technic.value.category_id,
            price_smena:form_technic.value.price_smena,
            price_hours:form_technic.value.price_hours,
            manufacturer:form_technic.value.manufacturer,
            img:form_technic.value.img
        },
        onResponseError({ request, response, options }) {
            alertStore.type = "danger"
        ...
        onResponse({ request, response, options }) {
            alertStore.type = "info"
            alertStore.message = "Техника успешно добавлена"
            alertStore.isAlert = true 
            form_technic.value = ''
            getTechnic()
        },
...
const { data: categorys } =  useFetch(`${config.public.apiBase}/categorys`,{
    method: 'get',
    headers: {
        Authorization: `Bearer ${accountStore.api_token}`,
    },
